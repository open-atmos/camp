    ...
    if (camp_mpi_rank() .eq. rank_solve) then
      ...
      do i_mass_frac = 0, NUM_MASS_FRAC_STEP
        ...
        if (i_mass_frac .eq. NUM_MASS_FRAC_STEP) then
          ...
          if (PHI_butanol > 0.0d0 .and. (1.0d0 - mole_frac) > 0.0d0 .and. THETA_butanol > 0.0d0) then
            if (PHI_butanol < (1.0d0 - mole_frac)) then
              ! You may want to check if division is less than 1, but log(x) is defined for any x > 0.
            end if
            ln_arg1 = PHI_butanol/(1.0d0 - mole_frac)
            ln_arg2 = THETA_butanol/PHI_butanol
            if (ln_arg1 > 0.0d0 .and. ln_arg2 > 0.0d0) then
              ln_gamma_C_butanol = log(ln_arg1) &
                                   + 5.0d0*q_butanol*log(ln_arg2) &
                                   + l_butanol &
                                   - PHI_butanol/(1.0d0 - mole_frac) &
                                   *((1.0d0 - mole_frac)*l_butanol + mole_frac*l_water)
            else
              ! Handle error: log argument not positive
              ln_gamma_C_butanol = 0.0d0
              print *, "Warning: log argument not positive in ln_gamma_C_butanol calculation."
            end if
          else
            ! Handle error: input values not in valid range
            ln_gamma_C_butanol = 0.0d0
            print *, "Warning: PHI_butanol, THETA_butanol, or denominator <= 0."
          end if
        end if
        if (i_mass_frac .eq. 0) then
          ln_gamma_C_water = 0.0d0
        else
          if (PHI_water > 0.0d0 .and. mole_frac > 0.0d0 .and. THETA_water > 0.0d0) then
            ln_arg1 = PHI_water/mole_frac
            ln_arg2 = THETA_water/PHI_water
            if (ln_arg1 > 0.0d0 .and. ln_arg2 > 0.0d0) then
              ln_gamma_C_water = log(ln_arg1) &
                                 + 5.0d0*q_water*log(ln_arg2) &
                                 + l_water &
                                 - PHI_water/mole_frac &
                                 *((1.0d0 - mole_frac)*l_butanol + mole_frac*l_water)
            else
              ln_gamma_C_water = 0.0d0
              print *, "Warning: log argument not positive in ln_gamma_C_water calculation."
            end if
          else
            ln_gamma_C_water = 0.0d0
            print *, "Warning: PHI_water, THETA_water, or mole_frac <= 0."
          end if
        end if
