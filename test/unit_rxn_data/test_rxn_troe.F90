...
!> \file
!> The camp_test_troe program

!> Test of troe reaction module
program camp_test_troe
  ...
  if (run_troe_tests()) then
    if (camp_mpi_rank().eq.0) write(*,*) "Troe reaction tests - PASS"
  else
    if (camp_mpi_rank().eq.0) write(*,*) "Troe reaction tests - FAIL"
    stop 3
  end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  logical function run_troe_test()
    ...
    if (camp_mpi_rank().eq.0) then
      ...
      call assert(145118257, camp_core%get_chem_spec_data(chem_spec_data))
