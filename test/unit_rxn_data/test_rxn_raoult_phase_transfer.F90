...
!> \file
!> The camp_test_raoult_phase_transfer program

!> Test of raoult_phase_transfer reaction module
program camp_test_raoult_phase_transfer
  ...
  if (run_raoult_phase_transfer_tests()) then
    if (camp_mpi_rank().eq.0) write(*,*) &
            "Raoult phase transfer reaction tests - PASS"
  else
    if (camp_mpi_rank().eq.0) write(*,*) &
            "Raoult phase transfer reaction tests - FAIL"
    stop 3
  end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  logical function run_raoult_phase_transfer_tests() result(passed)
    ...
    if (camp_solver_data%is_solver_available()) then
      passed = run_raoult_phase_transfer_test(1)
      passed = passed .and. run_raoult_phase_transfer_test(2)
    else
  ...
  end function run_raoult_phase_transfer_tests
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !! checker.
  logical function run_raoult_phase_transfer_test(scenario)
    ...
    run_raoult_phase_transfer_test = .true.
    ...
    if (camp_mpi_rank().eq.0) then
      ...
      if (scenario.eq.1) then
        input_file_path = 'test_raoult_phase_transfer_config.json'
      else if (scenario.eq.2) then
        input_file_path = 'test_raoult_phase_transfer_config_2.json'
      endif
    ...
    if (camp_mpi_rank().eq.rank_solve) then
      ...
      if (scenario.eq.1) then
        ...
        true_conc(0,idx_ethanol_aq) = true_conc(0,idx_ethanol_aq) / number_conc
        true_conc(0,idx_H2O_aq)     = true_conc(0,idx_H2O_aq)     / number_conc
        ! radius (m) calculated based on particle mass
        radius = ( ( true_conc(0,idx_ethanol_aq) / 1000.0 +  &
      ...
      ! Save the results
      if (scenario.eq.1) then
        open(unit=7, file="out/raoult_phase_transfer_results.txt", &
                status="replace", action="write")
      else if (scenario.eq.2) then
        open(unit=7, file="out/raoult_phase_transfer_results_2.txt", &
                status="replace", action="write")
      endif
      do i_time = 0, NUM_TIME_STEP
        write(7,'(7(ES13.6, 1X))') i_time*time_step, &
              true_conc(i_time, idx_ethanol),        &
