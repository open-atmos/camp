...
!> \file
!> The camp_test_wennberg_no_ro2 program

!> Test of wennberg_no_ro2 reaction module
program camp_test_wennberg_no_ro2
  ...
  if (run_wennberg_no_ro2_tests()) then
    if (camp_mpi_rank().eq.0) write(*,*) "Wennberg NO + RO2 reaction tests - PASS"
  else
    if (camp_mpi_rank().eq.0) write(*,*) "Wennberg NO + RO2 reaction tests - FAIL"
    stop 3
  end if
...
end program camp_test_wennberg_no_ro2
