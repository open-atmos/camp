...
int test_effective_radius(ModelData *model_data, N_Vector state) {
  ...
  aero_rep_get_effective_radius__m(model_data, AERO_REP_IDX, AERO_PHASE_IDX,
                                   &eff_rad, &(partial_deriv[1]));

  double volume_density =
      (CONC_1A / DENSITY_A + CONC_1B / DENSITY_B + CONC_1C / DENSITY_C +
       CONC_2C / DENSITY_C + CONC_2D / DENSITY_D + CONC_2E / DENSITY_E +
       CONC_3B / DENSITY_B + CONC_3E / DENSITY_E);  // volume density (m3/m3)
  double eff_rad_expected =
      pow((3.0 / 4.0 / 3.14159265359 * volume_density), 1.0 / 3.0);
  ret_val +=
      ASSERT_MSG(fabs(eff_rad - eff_rad_expected) < 1.0e-6 * eff_rad_expected,
                 "Bad effective radius");

  ret_val += ASSERT_MSG(partial_deriv[0] = 999.9, "Bad Jacobian (-1)");
  double d_eff_rad_dx =
...
int test_number_concentration(ModelData *model_data, N_Vector state) {
  ...
  aero_rep_get_number_conc__n_m3(model_data, AERO_REP_IDX, AERO_PHASE_IDX,
                                 &num_conc, &(partial_deriv[1]));

  ret_val +=
      ASSERT_MSG(fabs(num_conc - PART_NUM_CONC) < 1.0e-10 * PART_NUM_CONC,
                 "Bad number concentration");

  ret_val += ASSERT_MSG(partial_deriv[0] = 999.9, "Bad Jacobian (-1)");
  for (int i = 1; i < N_JAC_ELEM + 1; ++i)
    ret_val += ASSERT_MSG(partial_deriv[i] == ZERO, "Bad Jacobian element");
  ret_val +=
      ASSERT_MSG(partial_deriv[N_JAC_ELEM + 1] = 999.9, "Bad Jacobian (end+1)");
...
int test_aero_phase_mass(ModelData *model_data, N_Vector state) {
  ...
  for (int i = 1; i < 4; ++i)
    ret_val += ASSERT_MSG(partial_deriv[i] == ZERO, "Bad Jacobian element");
  for (int i = 4; i < 7; ++i)
    ret_val += ASSERT_MSG(partial_deriv[i] == ONE, "Bad Jacobian element");
...
int test_aero_phase_avg_MW(ModelData *model_data, N_Vector state) {
  ...
  aero_rep_get_aero_phase_avg_MW__kg_mol(
      model_data, AERO_REP_IDX, AERO_PHASE_IDX, &avg_mw, &(partial_deriv[1]));
  ...
  for (int i = 1; i < 4; ++i)
    ret_val += ASSERT_MSG(partial_deriv[i] == ZERO, "Bad Jacobian element");
  ret_val +=
      ASSERT_MSG(fabs(partial_deriv[4] - dMW_dC) < 1.0e-10 * fabs(dMW_dC),
                 "Bad Jacobian (-1)");
  ret_val +=
      ASSERT_MSG(fabs(partial_deriv[5] - dMW_dD) < 1.0e-10 * fabs(dMW_dD),
                 "Bad Jacobian (-1)");
  ret_val +=
      ASSERT_MSG(fabs(partial_deriv[6] - dMW_dE) < 1.0e-10 * fabs(dMW_dE),
                 "Bad Jacobian (-1)");
  for (int i = 7; i < N_JAC_ELEM + 1; ++i)
    ret_val += ASSERT_MSG(partial_deriv[i] == ZERO, "Bad Jacobian element");
  ret_val +=
 ...
 */
int run_aero_rep_single_particle_c_tests(void *solver_data, double *state,
                                         double *env) {
  int ret_val = 0;
  ...
  NV_DATA_S(solver_state)
  [(TEST_PARTICLE - 1) * 8 + 0] = state[(TEST_PARTICLE - 1) * 8 + 0] =
      CONC_1A;  // phase one, species a
  NV_DATA_S(solver_state)
  [(TEST_PARTICLE - 1) * 8 + 1] = state[(TEST_PARTICLE - 1) * 8 + 1] =
      CONC_1B;  // phase one, species b
  NV_DATA_S(solver_state)
  [(TEST_PARTICLE - 1) * 8 + 2] = state[(TEST_PARTICLE - 1) * 8 + 2] =
      CONC_1C;  // phase one, species c
  NV_DATA_S(solver_state)
  [(TEST_PARTICLE - 1) * 8 + 3] = state[(TEST_PARTICLE - 1) * 8 + 3] =
      CONC_2C;  // phase two, species c
  NV_DATA_S(solver_state)
  [(TEST_PARTICLE - 1) * 8 + 4] = state[(TEST_PARTICLE - 1) * 8 + 4] =
      CONC_2D;  // phase two, species d
  NV_DATA_S(solver_state)
  [(TEST_PARTICLE - 1) * 8 + 5] = state[(TEST_PARTICLE - 1) * 8 + 5] =
      CONC_2E;  // phase two, species e
  NV_DATA_S(solver_state)
  [(TEST_PARTICLE - 1) * 8 + 6] = state[(TEST_PARTICLE - 1) * 8 + 6] =
      CONC_3B;  // phase three, species b
  NV_DATA_S(solver_state)
  [(TEST_PARTICLE - 1) * 8 + 7] = state[(TEST_PARTICLE - 1) * 8 + 7] =
      CONC_3E;  // phase three, species e
