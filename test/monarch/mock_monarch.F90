...
program mock_monarch_t
  ...
  use camp_monarch_interfaceCPUGPU
  use camp_mpi
  use json_module
  ...
  type(camp_monarch_interfaceCPUGPU_t), pointer :: camp_interface
 ...
 "JSON not found at ", export_path
  ...
  camp_interface => camp_monarch_interfaceCPUGPU_t(camp_input_file, output_file_title, &
  ...
  !call set_env_equalCells(camp_interface,output_path)
  ...
  if (camp_mpi_rank() == 0) then
    print *, "num_steps(avg):", camp_interface%num_steps/(n_cells_monarch*NUM_TIME_STEP)
    print *, "max_steps:", camp_interface%max_steps
  end if
  if (camp_mpi_rank() == 0) then
    call camp_interface%camp_core%print_solver_stats() ! Useful for MONARCH
  end if
  if (camp_mpi_rank() == 0) then
...
contains
  subroutine set_env(camp_interface, file_prefix)
    type(camp_monarch_interfaceCPUGPU_t), intent(inout) :: camp_interface
  ...
  subroutine set_env_equalCells(camp_interface, file_prefix)
    type(camp_monarch_interfaceCPUGPU_t), intent(inout) :: camp_interface
    character(len=:), allocatable, intent(in) :: file_prefix
    character(len=:), allocatable :: file_name
    integer :: z, z1, o, i, j, k, r, t, i_cell, i_spec, mpi_size, ncells, tid, ncells_mpi
    ...
    print *, "DEBUG: Cells equal between them"
    temperature(:, :, :) = 290.016
    pressure(:, :, :) = 100000.
    allocate (camp_interface%rate_emi(24, n_cells_monarch))
    camp_interface%rate_emi(:, :) = 0.0
    do t = 1, 12 !12 first hours
      camp_interface%rate_emi(t, :) = 1.
    end do
    air_density(:, :, :) = pressure(:, :, :)/(287.04*temperature(:, :, :)* &
                                              (1.+0.60813824*water_conc(:, :, :, WATER_VAPOR_ID))) !kg m-3
    conv(:, :, :) = 0.02897/air_density(:, :, :)*(TIME_STEP*60.)*1e6 !units of time_step to seconds
