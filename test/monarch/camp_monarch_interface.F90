...
!> \file
!> The camp_monarch_interfaceCPUGPU_t object and related functions

!> Interface for the MONACH model and CAMP-camp
module camp_monarch_interfaceCPUGPU

  use camp_constants, only: i_kind
  use camp_mpi
  use camp_util, only: assert_msg, string_t, warn_assert_msg, to_string
  use camp_camp_core
  use camp_camp_state
  ...
  implicit none
  private
  public :: camp_monarch_interfaceCPUGPU_t
  type :: camp_monarch_interfaceCPUGPU_t
  ...
  interface camp_monarch_interfaceCPUGPU_t
...
contains
  ...
  function constructor(camp_config_file, output_file_title, &
    ...
    type(camp_monarch_interfaceCPUGPU_t), pointer :: this
    ...
    deallocate (buffer)
    call this%camp_core%solver_initialize(load_gpu=load_gpu, is_load_balance=is_load_balance)
    ...
    this%num_steps = 0
    this%max_steps = 0
    this%solver_stats => solver_stats_t(this%n_cells)
    if (MONARCH_PROCESS == 0) then
  ...
  subroutine integrate(this, curr_time, time_step, I_W, I_E, I_S, &
                       ...
                       NUM_TIME_STEP, i_time, load_gpu)
    class(camp_monarch_interfaceCPUGPU_t) :: this
    ...
    !print*,"id 21 conc:", this%camp_state%state_var(21)
    ...
    do i = I_W, I_E
      do j = I_S, I_N
        do k = 1, NUM_VERT_CELLS
          ...
          MONARCH_conc(i, j, k, this%map_monarch_id(:)) = &
            this%camp_state%state_var(this%map_camp_id(:) + (z*state_size_per_cell))
          this%num_steps = this%num_steps + this%solver_stats%num_steps(z)
          if (this%solver_stats%num_steps(z) > this%max_steps) then
            this%max_steps = this%solver_stats%num_steps(z)
          end if
        end do
      end do
    end do
  ...
  subroutine load_init_conc(this)
    ...
    !print*,"chem_spec_data%gas_state_id(PAR)",chem_spec_data%gas_state_id("PAR")
  ...
  elemental subroutine finalize(this)
    type(camp_monarch_interfaceCPUGPU_t), intent(inout) :: this
