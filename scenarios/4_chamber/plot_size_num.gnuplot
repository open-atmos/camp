...
plot "out/chamber_00000001_num_dist.txt" using ($1*1e9):($2/1e6):($3/1e6) with errorbars title "CAMP simulation, t = 0 min", \
     "out/chamber_00000002_num_dist.txt" using ($1*1e9):($2/1e6):($3/1e6) with errorbars title "CAMP simulation, t = 7 min", \
     "out/chamber_00000011_num_dist.txt" using ($1*1e9):($2/1e6):($3/1e6) with errorbars title "CAMP simulation, t = 70 min", \
     "out/chamber_00000031_num_dist.txt" using ($1*1e9):($2/1e6):($3/1e6) with errorbars title "CAMP simulation, t = 210 min", \
