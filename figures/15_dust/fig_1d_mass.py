...
def make_plot(in_dir, in_filename1, in_filename2, in_filename3, out_filename):
    ...
    hist1 = camp.histogram_1d(dry_diameters1, x_axis, weights = particles1.masses(exclude=["H2O"])/ particles1.comp_vols)
    hist2 = camp.histogram_1d(dry_diameters2, x_axis, weights = particles2.masses(exclude=["H2O"]) / particles2.comp_vols)
    hist3 = camp.histogram_1d(dry_diameters3, x_axis, weights = particles3.masses(exclude=["H2O"]) / particles3.comp_vols)
