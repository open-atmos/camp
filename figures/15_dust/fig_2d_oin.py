...
def make_plot(in_filename,out_filename,title):
    ...
    x_axis = camp.log_grid(min=1e-8,max=1e-4,n_bin=100)
    y_axis = camp.log_grid(min=1e-19,max=6e-15,n_bin=30)

    hist2d = camp.histogram_2d(dry_diameters, oin, x_axis, y_axis, weights = 1/particles.comp_vols)
    plt.clf()
