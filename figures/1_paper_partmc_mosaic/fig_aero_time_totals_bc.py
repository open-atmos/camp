...
for use_color in [True, False]:
    ...
    for i in range(len(plots)):
        ...
        for [time, filename, key] in time_filename_list:
            ...
            if plots[i]["mass"]:
                if plots[i]["wet"]:
                    values = (particles.mass() / array(particles.comp_vol)) * 1e9
                else:
                    values = (particles.mass(exclude = ["H2O"])
                             / array(particles.comp_vol)) * 1e9
            else:
                values = (1.0 / array(particles.comp_vol)) * 1e-6 # m^{-3} to cm^{-3}
            soot_mass = particles.mass(include = ["BC"])
            value = (values[soot_mass > 0.0]).sum()
