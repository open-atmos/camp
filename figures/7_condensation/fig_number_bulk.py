...
netcdf_dir = "/home/lfierce2/subversion/camp/trunk/urban_plume/data/scaleBC3/rate_10k/"
...
for [time, filename, key] in time_filename_list:
    ...
    h2o = sum(particles.masses(include = ["H2O"])/particles.comp_vols)
    ...
    array_wc[i_counter,9]= h2o
    
    i_counter += 1

np.savetxt("species.txt", array_wc)
...
fig = plt.gcf()
fig.savefig("figs/number_laura_rate_10k.pdf")
...
fig = plt.gcf()
fig.savefig("figs/mass_laura_rate_10k.pdf")
...
plt.plot(array_wc[:,0], array_wc[:,9], 'b--', label = 'h2o wc')
plt.legend(loc = 'upper right')
plt.xlabel("time ")
plt.ylabel("mass concentration in kg m^{-3}")
fig = plt.gcf()
fig.savefig("figs/mass_species_laura_rate_10k.pdf")
...
#plt.plot(array_nc[:,0], array_nc[:,9], 'm-', label = 'h2o nc')
#plt.legend(loc = 'upper right')
#plt.xlabel("time ")
#plt.ylabel("mass concentration in kg m^{-3}")
#fig = plt.gcf()
