    ...
    for coag in [True]:
        ...
        for i in range(size(diameter)):
            ...
            if plot_info["criterion"] == "none":
                ...
                if time_emitted_array[id] < plot_info["time_start"]:
                    accept = False
                if time_emitted_array[id] > plot_info["time_end"]:
                    accept = False
            elif plot_info["criterion"] == "aging_time":
                aging_time_point = time_emitted_array[id] + time_aging_array[id]
                if aging_time_point < plot_info["time_start"]:
                    accept = False
                if aging_time_point > plot_info["time_end"]:
                    accept = False
            else:
                raise Exception("unknown criterion: %s" % plot_info["criterion"])
            if accept and (bc_mass[i] > 0.0):
