...
x_array = np.loadtxt("data/1d_brown_x_values.txt") * 1e6
num_avg1 = np.loadtxt("data/1d_brown_hist_array_gav_num.txt") / 1e6
num_std1 = np.loadtxt("data/1d_brown_e_bars_num.txt") / 1e6
...
(figure, axes_array) = mpl_helper.make_fig_array(1,2, figure_width=config.figure_width_double, 
                                                 ...
                                                 left_margin=0.7, right_margin=0.6,
                                                 horiz_sep=0.3, vert_sep=0.3,
                                                 share_y_axes=False)

axes = axes_array[0][0]
axes.plot(sect_array_num[:,0]*1e6, sect_array_num[:,12]*np.log(10)/1e6, 'b-')
axes.errorbar(x_array, num_avg1, conf_factor*num_std1, fmt=None, ecolor='r')
axes.set_xscale("log")
...
axes.errorbar(x_array, mass_avg1, conf_factor*mass_std1, fmt=None, ecolor='r')
axes.set_xscale("log")
axes.set_yscale("linear")
