...
out_prefix = "figs/mp_2d_soa_carbon"
...
def get_plot_data(filename, value_min = None, value_max = None):
    ...
    x_axis = camp.linear_grid(min = soa_axis_min, max = soa_axis_max,
                                n_bin = num_soa_bins * 2)
    y_axis = camp.linear_grid(min = oc_axis_min, max = oc_axis_max,
                                n_bin = num_oc_bins * 2)

    value = camp.multival_2d(comp_frac_soa, comp_frac_ocbc, diameters, x_axis, y_axis)
    if value_max == None:
        value_max = value.max()
    if value_min == None:
...
def make_2d_plot(in_filename, out_filename):
    ...
    axes.set_xbound(soa_axis_min, soa_axis_max)
    ...
    axes.set_xlabel(r"SOA dry mass frac. $w_{{\rm SOA},{\rm dry}}\ (\%)$")
