...
def get_plot_data_bc(filename, value_min = None, value_max = None):
    ...
    comp_frac_bc = particles.masses(include = ["BC"]) \
        / particles.masses(exclude = ["H2O"]) * 100
    comp_frac_oc = particles.masses(include = ["OC"]) \
        / particles.masses(exclude = ["H2O"]) * 100
    # hack to avoid landing just around the integer boundaries
    comp_frac_bc *= (1.0 + 1e-12)
    comp_frac_oc *= (1.0 + 1e-12)
    h2o = particles.masses(include = ["H2O"])

    x_axis = camp.linear_grid(min = bc_axis_min, max = bc_axis_max,
                                n_bin = num_bc_bins * 2)
    y_axis = camp.linear_grid(min = oc_axis_min, max = oc_axis_max,
                                n_bin = num_oc_bins * 2)

    value = camp.multival_2d(comp_frac_bc, comp_frac_oc, diameters, x_axis, y_axis)
    if value_max == None:
        value_max = value.max()
    if value_min == None:
...
def make_2d_plot(in_filename, out_filename):
    ...
    axes.set_xbound(bc_axis_min, bc_axis_max)
    ...
    axes.set_xlabel(r"BC dry mass frac. $w_{{\rm BC},{\rm dry}}\ (\%)$")
    axes.set_ylabel(r"OC dry mass frac. $w_{{\rm OC},{\rm dry}}\ (\%)$")
