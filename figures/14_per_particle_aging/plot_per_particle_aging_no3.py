...
p = axes.scatter(emit_diam[emit_morning], time_for_aging[emit_morning], c=aging_no3_fraction[emit_morning], 
...
cbar.set_label(r"$w_{\rm NO_3}$ at time of aging")

axes = axes_array[1][0]
p = axes.scatter(emit_diam[emit_afternoon], time_for_aging[emit_afternoon], c=aging_no3_fraction[emit_afternoon], 
		 norm = matplotlib.colors.Normalize(vmin=0, vmax=1), s=2, linewidths=0)
axes.set_xscale("log")
axes.set_yscale("linear")
axes.set_ylim(0, 15)
...
axes = axes_array[0][0]
p = axes.scatter(emit_diam[emit_night], time_for_aging[emit_night], c=aging_no3_fraction[emit_night],
		 norm = matplotlib.colors.Normalize(vmin=0, vmax=1), s=2, linewidths=0)
axes.set_xscale("log")
axes.set_yscale("linear")
axes.set_ylim(0, 15)
