...
!> \file
!> The camp_rxn_raoult_phase_transfer module.

!> \page camp_rxn_raoult_phase_transfer CAMP: Raoult Phase-Transfer Reaction
!!
!! Raoult phase transfer reactions are based on the Raoult's law phase transfer.
!!
!! Mass accomodation coefficients and condensation rate constants are
!! calculated using the method of Ervens et al. (2003) \cite Ervens2003 and
...
!!
!! The key-value pair \b pvap is also required, which specifies the saturation vapor
!! pressure.
!! The key_value pair \b pvap_ref_temp [K] is optional, it specifies the pvap
!! reference temperature for the temperature dependence calculation.
!! By default it is set at 298 K.
!! The key-value pair \b pvap_unit is optional. Its possible values are
!!  "atm", "hPa", "Pa", "C*_ug_m3". If using unit C*_ug_m3, the key-value
!! pair \b C*_ref_temp \b [\b K] is required, and \b C*_ref_act_coef is optional,
!! assumed to be 1.0 if not provided
!! The key-value pair \b delta_h_vap \b [\b 1/K] is required for the latent heat
!! of vaporization.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

!> The rxn_raoult_phase_transfer_t type and associated functions.
module camp_rxn_raoult_phase_transfer
...
#define PVAP_ this%condensed_data_real(5)
#define PVAP_REF_TEMP_ this%condensed_data_real(6)
#define DELTA_H_VAP_ this%condensed_data_real(7)
#define CONV_ this%condensed_data_real(8)
#define MW_ this%condensed_data_real(9)
...
#define MW_JAC_ELEM_(x,e) this%condensed_data_real(PHASE_REAL_LOC_(x)-1+3*NUM_AERO_PHASE_JAC_ELEM_(x)+e)
  ...
  public :: rxn_raoult_phase_transfer_t

  !> Generic test reaction data type
  type, extends(rxn_data_t) :: rxn_raoult_phase_transfer_t
  contains
    !> Reaction initialization
    procedure :: initialize
    !> Finalize the reaction
    final :: finalize
  end type rxn_raoult_phase_transfer_t

  !> Constructor for rxn_SIMPOL_phase_transfer_t
  interface rxn_raoult_phase_transfer_t
    procedure :: constructor
  end interface rxn_raoult_phase_transfer_t
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function constructor() result(new_obj)
    ...
    type(rxn_raoult_phase_transfer_t), pointer :: new_obj
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine initialize(this, chem_spec_data, aero_rep, n_cells)
    ...
    !> Reaction data
    class(rxn_raoult_phase_transfer_t), intent(inout) :: this
    ...
    real(kind=dp) :: pvap, cstar_ref_temp, cstar_ref_act_coef
    character(len=:), allocatable :: key_name, gas_spec_name, aero_spec_name
    character(len=:), allocatable :: pvap_unit
    ...
    call assert_msg(215991110, associated(aero_rep), &
      "Missing aerosol representation"//error_msg)
    call assert_msg(440626195, size(aero_rep) .gt. 0, &
      "Missing aerosol representation"//error_msg)
    ...
    call assert_msg(989923349, n_aero_ids .gt. 0, &
    ...
    call assert_msg(991843640, spec_props%get_real(key_name, MW_), &
    ...
    do i_aero_rep = 1, size(aero_rep)
      ...
      if (has_act_coeff) then
        ...
        call assert_msg(621176148, size(unique_act_names) .eq. &
          size(unique_spec_names), &
          "Mismatch of raoult species and activity coeffs"// &
          error_msg)
      end if
    ...
    ! Get the Raout parameters
    key_name = "pvap"
    call assert_msg(621176148, &
      this%property_set%get_real(key_name, pvap), &
      "Missing 'pvap' parameter"//error_msg)

    key_name = "pvap_unit"
    if (.not. this%property_set%get_string(key_name, pvap_unit)) then
      pvap_unit = "atm"
    end if

    ! convert to Pa
    select case (pvap_unit)
     case ("atm")
      pvap = const%air_std_press*pvap
     case ("Pa")
      pvap = pvap
     case ("hPa")
      pvap = 100.0*pvap
     case ("C*_ug_m3")
      key_name = "C*_ref_temp"
      call assert_msg(972384110, &
        this%property_set%get_real(key_name, cstar_ref_temp), &
        "Missing 'C*_ref_temp' parameter when using C*_ug_m3 unit. "//error_msg)

      key_name = "C*_ref_act_coef"
      if (.not. this%property_set%get_real(key_name, cstar_ref_act_coef)) then
        cstar_ref_act_coef = 1.0
      end if

      pvap = pvap*const%univ_gas_const*cstar_ref_temp/ &
        (MW_*1e9*cstar_ref_act_coef) ! MW_ is in kg/mol!
     case default
      call die_msg(847721259, "Unknown pvap_unit: "//pvap_unit//". "//error_msg)
    end select
    PVAP_ = pvap

    key_name = "pvap_ref_temp"
    if (.not. this%property_set%get_real(key_name, PVAP_REF_TEMP_)) then
      PVAP_REF_TEMP_ = 298.0
    end if

    key_name = "delta_h_vap"
    call assert_msg(541447462, &
      this%property_set%get_real(key_name, DELTA_H_VAP_), &
      "Missing 'delta_h_vap' parameter. "//error_msg)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  elemental subroutine finalize(this)
    ...
    !> Reaction data
    type(rxn_raoult_phase_transfer_t), intent(inout) :: this
...
end module camp_rxn_raoult_phase_transfer
