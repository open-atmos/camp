...
#define PVAP_ float_data[4]
#define PVAP_REF_TEMP_ float_data[5]
#define DELTA_H_VAP_ float_data[6]
#define CONV_ float_data[7]
#define MW_ float_data[8]
...
void rxn_raoult_phase_transfer_get_used_jac_elem(ModelData *model_data,
  ...
  if (aero_jac_elem == NULL) {
    printf("\n\nERROR allocating space for 1D Jacobian structure array for "
           "raoult phase transfer reaction\n\n");
    exit(1);
  }
  ...
  for (int i_aero_phase = 0; i_aero_phase < NUM_AERO_PHASE_; i_aero_phase++) {
    ...
    if (n_jac_elem > NUM_AERO_PHASE_JAC_ELEM_(i_aero_phase)) {
      printf(
          "\n\nERROR Received more Jacobian elements than expected for raoult "
    ...
    if (i_used_elem != n_jac_elem) {
      printf("\n\nERROR setting used Jacobian elements in raoult phase "
...
void rxn_raoult_phase_transfer_update_env_state(ModelData *model_data,
  ...
  double vp =
      PVAP_ * exp(DELTA_H_VAP_ * (1.0 / PVAP_REF_TEMP_ - 1.0 / TEMPERATURE_K_));
 ...
 * \param rxn_float_data Pointer to the reaction floating-point data
 */
void rxn_raoult_phase_transfer_print(int *rxn_int_data,
                                     double *rxn_float_data) {
  int *int_data = rxn_int_data;
  double *float_data = rxn_float_data;
