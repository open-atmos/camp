...
__device__
int cudaDevicecvEwtSetSV2(ModelDataGPU *md, ModelDataVariable *dmdv,
  ...
  double min;
#ifdef DEV_DC
 cudaDevicezaxpby2(md->cv_reltol, md->dtempv, 1.,
...
__device__
int cudaDeviceCVode2(ModelDataGPU *md, ModelDataVariable *dmdv) {
  ...
  for(;;) {
    ...
    int ewtsetOK = 0;
    if (md->s->cv_nst > 0) {
      ewtsetOK = cudaDevicecvEwtSetSV2(md, dmdv, md->dzn, md->dewt);
      if (ewtsetOK != 0) {
...
#else
  istate=cudaDeviceCVode2(md,md->s);
#endif
    __syncthreads();
#ifdef CAMP_DEBUG_GPU
#ifdef CAMP_PROFILE_DEVICE_FUNCTIONS
