...
__device__
int cudaDevicecamp_solver_check_model_state(ModelDataGPU *md, double *y)
{
...
int CudaDeviceguess_helper(double t_n, double h_n, double* y_n,
                           ...
                           double* dtempv2, double cv_reltol,
                           ModelDataGPU *md
) {
  //SolverData *sd = (SolverData *)solver_data;
  //realtype *ay_n = NV_DATA_S(y_n);
  //realtype *ay_n1 = NV_DATA_S(y_n1);
  //realtype *adtempv1 = NV_DATA_S(dtempv1);
  //realtype *acorr = NV_DATA_S(dtempv2);
  ...
  // Get  \f$f(t_{n-1})\f$
  /*if (h_n > ZERO) {
    N_VScale(ONE / h_n, hf, dtempv2);
  } else {
    N_VScale(ONE, hf, dtempv2);
  }*/

  if (h_n > 0.) {
    //cudaDevicescalezy(1./h_n, hf, dtempv2, nrows);
    dtempv2[i]=(1./h_n)*hf[i];
  } else {
    //cudaDevicescalezy(1., hf, dtempv2, nrows);
    dtempv2[i]=hf[i];
  }
  ...
  for (int iter = 0; iter < GUESS_MAX_ITER && t_0 + t_j < t_n; iter++) {
    ...
    for (int i = 0; i < n_elem; i++) {
     realtype t_star = -adtempv1[i] / acorr[i];
    ...
    if(tid==0){
      for (int j = 0; j < md->nrows; j++) {
        t_star = -dtempv1[j] / dtempv2[j];
        if ((t_star > ZERO || (t_star == ZERO && dtempv2[j] < ZERO)) &&
            t_star < h_j) {
          h_j = t_star;
        }
      }
...
/*
    __syncthreads();
    double t_star = -dtempv1[i] / dtempv2[i];
    if(t_star < 0. || (t_star == 0. && dtempv2[i] > 0.)) {
      //t_star=999999;
      t_star = h_j;
    }else{
...
*/
    ...
    //         i,dtempv2[i],hf[i],t_star,h_j,h_n);
    if(i==0)
      for(int j=0;j<nrows;j++)
        printf("dcorr[%d] %le ddtempv1 %le dhf %le dt_star %le dh_j %le dh_n %le\n",
               j,dtempv2[j],dtempv1[j],hf[j],t_star,h_j,h_n);

#endif

    cudaDevicef(
            t_0 + t_j, md->deriv_length_cell, md->state_size_cell,
            md->n_cells, md->i_kernel, md->threads_block,
            md->n_shr_empty, md->dtempv1,
            md->dtempv2, md
    );

    __syncthreads();
    if (*md->flag != 0) {//CAMP_SOLVER_FAIL
      //N_VConst(ZERO, dtempv2);
      dtempv2[i] = 0.;
      flag_shr2[0]=-1;
#ifdef DEBUG_CudaDeviceguess_helper
  ...
  //if (h_n > 0.) N_VScale(0.999, dtempv2, dtempv2);
  if (h_n > 0.) dtempv2[i]=dtempv2[i]*0.999;
...
//#define SUNMAX(A, B) ((A) > (B) ? (A) : (B))
    ...
    //gpu_zaxpby(1.0, mGPU->dcv_y, -1.0, mGPU->dzn, mGPU->dacor, mGPU->nrows, mGPU->blocks, mGPU->threads);
    __syncthreads();
    ...
    //*cv_nfe++;
    if (i == 0)mdv->cv_nfe=mdv->cv_nfe+1;
...
__device__
void cudaDevicecvNlsNewton(ModelDataGPU *md)
{
  ModelDataVariable *mdv = md->mdv;
  extern __shared__ int flag_shr[];
  flag_shr[0] = 0;
  ...
  int callSetup = (md->nflag == PREV_CONV_FAIL) || (md->nflag == PREV_ERR_FAIL) ||
                  (*md->cv_nst == 0) ||
                  (*md->cv_nst >= md->cv_nstlp + MSBP) ||
                  (dgamrat > DGMAX);

  md->dftemp[i]=md->dzn[i]*(-md->cv_last_yn[i]);
  __syncthreads();
  int guessflag=CudaDeviceguess_helper(md->t_n, md->h_n, md->dzn,
                                       md->cv_last_yn, md->dftemp, md->dtempv,
                                       md->cv_acor_init, md->cv_reltol, md
  );
  __syncthreads();
...
*/
  ...
  for(;;) {
    ...
    cudaDevicef(
            ...
            md->n_cells, md->i_kernel, md->threads_block, md->n_shr_empty, md->dcv_y,
            md->dftemp,md
    );
    ...
    if (callSetup) {
      ...
      cudaDevicelinsolsetup(
              ...
              //cudacvNewtonIteration
              md->dacor, md->dzn,
              md->dewt, md->dcv_tq, &mdv->cv_nfe,
              &mdv->cv_nsetups,
              md->cv_nst, &mdv->nstlj, convfail,
              md->isavedJ, md->jsavedJ, md->savedJ,
              &mdv->nje
#ifdef PMC_DEBUG_GPU
      ,&mdv->counterBCGInternal, &mdv->dtBCG, &mdv->dtPreBCG, &mdv->dtPostBCG,
          mdv->counterDerivGPU
#endif
      );
      __syncthreads();
                       ...
                       SolverData *sd, CVodeMem cv_mem)
{
  ...
  // default value
  mGPU->time_step = time_step > 0. ? time_step : sd->init_time_step;
...
int cvNlsNewton_gpu2(SolverData *sd, CVodeMem cv_mem, int nflag)
{
  ...
  for(;;) {
    ...
    //gpu_zaxpby(1.0, mGPU->dzn, 1.0, mGPU->dacor_init, mGPU->dcv_y, mGPU->nrows, mGPU->blocks, mGPU->threads);
    cudaDeviceSynchronize();
    ...
    //gpu_yequalsx(mGPU->dacor, mGPU->dacor_init, mGPU->nrows, mGPU->blocks, mGPU->threads);
...
int linsolsetup_gpu2(SolverData *sd, CVodeMem cv_mem,int convfail,N_Vector vtemp1,N_Vector vtemp2,N_Vector vtemp3)
{
  ...
  if (jok) {
    ...
    //        N_Vector dtempv1, N_Vector tmp2, N_Vector tmp3) {
...
int cvNewtonIteration_gpu2(SolverData *sd, CVodeMem cv_mem)
{
  ...
  for(;;) {
    ...
    // a*x + b*y = z
    gpu_zaxpby(cv_mem->cv_rl1, (mGPU->dzn+1*mGPU->nrows), 1.0, mGPU->dacor, mGPU->dtempv, mGPU->nrows, mGPU->blocks, mGPU->threads);
  ...
  if(bicg->counterBiConjGrad==0){
    ...
    // a*x + b*y = z
    gpu_zaxpby(1.0, mGPU->dacor, 1.0, mGPU->dx, mGPU->dacor, mGPU->nrows, mGPU->blocks, mGPU->threads);
    gpu_zaxpby(1.0, mGPU->dzn, 1.0, mGPU->dacor, mGPU->dcv_y, mGPU->nrows, mGPU->blocks, mGPU->threads);
