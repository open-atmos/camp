...
,int *it_pointer, double *dtBCG, double *dtPreBCG, double *dtPostBCG,
  ...
  //if(i==0)printf("G %d %d %d %lf\n",md->nflag,*cv_nst,md->cv_nstlp,md->cv_gamrat);
 ...
 */
  ...
  for(;;) {
    ...
    callSetup = 1;
    __syncthreads();
    if (i == 0) callSetupGlobal = 1;//todo delete
