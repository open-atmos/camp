...
//Some functs
  ...
  cudaDeviceswapCSC_CSR(nrows, nrows, diA, djA, dA, diB, djB, dB);
    ...
    solveBcgCudaDeviceCVODE( //Works with Multi-cells
            ...
            tolmax, ddiag, dr0, dr0h, dn0, dp0, dt, ds, dAx2, dy, dz, md, dmdv
    );
...
#ifdef DEBUG_CudaDeviceguess_helper
    if(i==0)printf("CudaDeviceguess_helper cudaDevicecvNewtonIteration2 start\n");
#endif
...
int cudaDevicecvStep(ModelDataGPU *md, ModelDataVariable *dmdv) {
  ...
  for (;;) {
    ...
    nflag = cudaDevicecvNlsNewton(
            ...
            //cudacvNewtonIteration
            md->cv_acor, md->dzn0, md->dewt, md->cv_tq,
...
__device__
int cudaDevicecvTemplate(ModelDataGPU *md, ModelDataVariable *dmdv) {
...
int cudaDeviceCVode(ModelDataGPU *md, ModelDataVariable *dmdv) {
  ...
  *mdv2 = *dmdv; //fine, but why? what is left to copy? debug?
  ...
  //if(threadIdx.x==0)printf("cudaDeviceCVode HPRIME2 %le HPRIME3 %le block %d\n",
  //        dmdv->cv_hprime2,dmdv->cv_hprime3,blockIdx.x);
  ...
  __syncthreads();
    ...
    if(i==0){
      ...
      printf("dmdv->cv_tn %le dmdv->tout %le dmdv->cv_h %le\n",
             dmdv->cv_tn,dmdv->tout,dmdv->cv_h);
...
/*
      //wrong?, make fails cv_hprime update to dmdv
