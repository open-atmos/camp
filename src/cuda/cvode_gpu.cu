...
int cudaDevicecamp_solver_check_model_state(ModelDataGPU *md, ModelDataVariable *dmdv, double *y, int *flag)
{
...
#else
    //flag_shr[0] = *flag;
    //if(i==0)flag_shr2[0] = 0;//wrong, but why
    flag_shr2[0] = 0;
#endif
    __syncthreads();
  ...
  *flag=max;
  ...
  //flag_shr2[0] = aux_flag;
  //__syncthreads();
  //if(flag_shr2[0]==CAMP_SOLVER_FAIL)*flag=CAMP_SOLVER_FAIL;
  //__syncthreads();
  ...
  *flag = (int)flag_shr2[0];
  ...
  //cudaDevicemaxI(&max, aux_flag, flag_shr2, md->n_shr_empty);//Re-calculating the value prints fine
  //dmdv->flag=max;//wrong printing
...
) //Interface CPU/GPU
{
  ...
  if(tid<active_threads){
    ...
    //wrong 1000 cells (so it needs sync somehow)
    cudaDeviceswapCSC_CSR(md->nrows, md->nrows, md->iJ_solver, md->jJ_solver, md->J_solver, md->diB, md->djB,md-> dB);
    cudaDeviceSpmv(md->J_tmp2, md->J_tmp, active_threads, md->J_solver, md->jJ_solver, md->iJ_solver, md->n_shr_empty);
    cudaDeviceswapCSC_CSR(md->nrows, md->nrows, md->iJ_solver, md->jJ_solver, md->J_solver, md->diB, md->djB,md-> dB);
    ...
    printmin(md,yout,"cudaDevicecalc_deriv start end yout");//fine with check_model_state afte
...
) //Interface CPU/GPU
{
  ...
  printmin(md,yout,"cudaDevicef End yout");//fine
  ...
  //if(i==0)printf("cudaDevicef blockIdx.x %d\n", blockIdx.x);//fine
  //if(i==0)printf("cudaDevicef checkflag %d *flag %d\n",checkflag, *flag);//fine
  //printmin(md,yout,"cudaDevicef End yout");//fine
  //printmin(md,y,"cudaDevicef End y");//fine
...
) //Interface CPU/GPU
{
  ...
  //printmin(md,y,"cudaDeviceJac end y");//fine
  //printmin(md,dftemp,"cudaDeviceJac end dftemp");//fine

  printmin(md,md->state,"cudaDeviceJac start state");//fine
  ...
  //__syncthreads();
  //printmin(md,dftemp,"cudaDeviceJac end dftemp");//wrong
...
//#ifndef DEV_DZN0
//#else //Seems dzn is already removed on cudaDevice
...
//#endif
  ...
  cudaMemcpy(mGPU->flag2,&flag,1*sizeof(int),cudaMemcpyHostToDevice);
...
//Some functs
...
#ifndef DEV_DZN0
    cudaDevicezaxpby(dmdv->cv_rl1, md->dzn1, 1.0, cv_acor, dtempv, nrows);
#else
  ...
  cudaDeviceswapCSC_CSR(nrows, nrows, diA, djA, dA, diB, djB, dB);
    ...
    if(i==0) dmdv->dtBCG += ((double)(int)(clock() - start))/(clock_khz*1000);
...
*/
    ...
    cudaDevicescaley(dznj,factor,md->nrows);
...
void cudaDevicecvRestore(ModelDataGPU *md, ModelDataVariable *dmdv, double saved_t) {
  ...
  for (k = 1; k <= dmdv->cv_q; k++){
    for (j = dmdv->cv_q; j >= k; j--) {
      ...
      dzn[tid]=1*dzn[tid]+(-1)*md->dzn[md->nrows*(j)+i];

      md->dzn[md->nrows*(j-1)+i]=dzn[tid];
      __syncthreads();
...
void cudaDevicecvPredict(ModelDataGPU *md, ModelDataVariable *dmdv) {
  ...
  for (k = 1; k <= dmdv->cv_q; k++){
    ...
    for (j = dmdv->cv_q; j >= k; j--){
      ...
      dzn[tid]=md->dzn[md->nrows*(j)+i]+dzn[tid];
...
int cudaDevicecvDoErrorTest(ModelDataGPU *md, ModelDataVariable *dmdv,
  ...
  cudaDevicezaxpby(md->cv_l[0+blockIdx.x*L_MAX],
    md->cv_acor, 1., md->dzn0, md->dftemp, md->nrows);
#else
  cudaDevicezaxpby(md->cv_l[0+blockIdx.x*L_MAX],
...
/*
    ...
    if(threadIdx.x==0) {
      for(int n=md->nrows/gridDim.x*blockIdx.x;n<md->nrows/gridDim.x*(blockIdx.x+1);n++){
      //for(int n=0;n<md->nrows;n++){
        dzn[n]=-md->cv_l[0+blockIdx.x*L_MAX]*md->cv_acor[n]+md->dftemp[n];
      }
    }__syncthreads();
...
#ifndef DEV_DZN0
    cudaDevicezaxpby(-md->cv_l[0+blockIdx.x*L_MAX],
      md->cv_acor, 1., md->dftemp, md->dzn0, md->nrows);
#else
    cudaDevicezaxpby(-md->cv_l[0+blockIdx.x*L_MAX],
  ...
  *nflagPtr = PREV_ERR_FAIL;
...
#ifndef DEBUG_cudaDevicef
  //printmin(md,md->state,"cudaDevicef start state");//fine
#endif
  ...
  retval=cudaDevicef(
            ...
            md->dzn0, md->dtempv,md,dmdv, &aux_flag
  );
...
#endif
  //printmin(md,md->dzn0,"cudaDevicef end dzn0");//fine
  ...
  dzn[tid]=dmdv->cv_h*md->dtempv[i];

  md->dzn[md->nrows+i]=dzn[tid];
  __syncthreads();

#else
  //N_VScale(cv_mem->cv_h, cv_mem->cv_tempv, cv_mem->cv_zn[1]);
#ifndef DEV_DZN0
    md->dzn1[i]=dmdv->cv_h*md->dtempv[i];
#else
...
void cudaDevicecvCompleteStep(ModelDataGPU *md, ModelDataVariable *dmdv) {
  ...
  if ((dmdv->cv_qwait == 1) && (dmdv->cv_q != dmdv->cv_qmax)) {
    ...
    dzn[tid]=1.*md->cv_acor[i];

    md->dzn[md->nrows*(dmdv->cv_qmax)+i]=dzn[tid];
    __syncthreads();
#else
...
int cudaDevicecvPrepareNextStep(ModelDataGPU *md, ModelDataVariable *dmdv, double dsm) {
  ...
  if (dmdv->cv_q > 1) {
    ...
    //atomicMin(&sdata[1],ddn);
    //ddn=sdata[1];

    //atomicAdd(&md->J_tmp[1],ddn/gridDim.x); //wrong eta 1000 cells
    //ddn=md->J_tmp[1];
  ...
  if (dmdv->cv_q != dmdv->cv_qmax && dmdv->cv_saved_tq5 != 0.) {
    ...
    //atomicAdd(&md->J_tmp[1],dup/gridDim.x); //fine no cv eta 1000 cells
    //dup=md->J_tmp[1];
...
void cudaDevicecvIncreaseBDF(ModelDataGPU *md, ModelDataVariable *dmdv) {
  ...
  md->dzn[md->nrows*(dmdv->cv_L)+i]=
          A1*md->dzn[md->nrows*(dmdv->cv_indx_acor)+i];
#endif
...
int cudaDevicecvStep(ModelDataGPU *md, ModelDataVariable *dmdv) {
  ...
  for (;;) {
    ...
    nflag = cudaDevicecvNlsNewton(
            ...
            md->dtempv2, dmdv->cv_reltol,
            //update_state
            md->threshhold, md->replacement_value, &dmdv->nflag,
            //f_gpu
            dmdv->cv_next_h, md->deriv_length_cell, md->state_size_cell,
            md->i_kernel, md->threads_block, md, dmdv,
            //cudacvNewtonIteration
...
*/
 //  dmdv->cv_etamax = (dmdv->cv_nst <= SMALL_NST) ? ETAMX2 : ETAMX3;
  ...
  //if(i==0)printf("DEV_cudaDevicecvStep dmdv->cv_etamax %le dmdv->cv_nst %d\n",
...
#ifndef DEV_DZN0
      ewtsetOK = cudaDevicecvEwtSetSV(md, dmdv, md->dzn0, md->dewt);//wrong
#else
  ...
  *mdv2 = *dmdv; //fine, but why? what is left to copy? debug?
  ...
  __syncthreads();
    ...
    if(i==0){
      ...
      printf("DEV_cudaDeviceCVode counterBiConjGrad %d thread %d\n",counterBiConjGrad, i);
      //printf("dmdv->kflag %d dmdv->istate %d\n",dmdv->kflag, dmdv->istate);
      ...
      //printf("dmdv->cv_tn %le dmdv->tout %le dmdv->cv_h %le cond %le\n",
    ...
    if (dmdv->kflag2 != CV_SUCCESS) {
      ...
      if(i==0) printf("dmdv->kflag != CV_SUCCESS istate %d\n",dmdv->istate);
...
*/
...
#ifndef DEV_DZN0
    if(i==0){
      cudaMemcpy(mGPU->dzn0, zn, mGPU->nrows * sizeof(double), cudaMemcpyHostToDevice);
    }else if(i==1){
      cudaMemcpy(mGPU->dzn1, zn, mGPU->nrows * sizeof(double), cudaMemcpyHostToDevice);
    }else if(i==2){
      cudaMemcpy(mGPU->dzn2, zn, mGPU->nrows * sizeof(double), cudaMemcpyHostToDevice);
    }else if(i==3){
      cudaMemcpy(mGPU->dzn3, zn, mGPU->nrows * sizeof(double), cudaMemcpyHostToDevice);
    }else if(i==4){
      cudaMemcpy(mGPU->dzn4, zn, mGPU->nrows * sizeof(double), cudaMemcpyHostToDevice);
    }else if(i==5){
      cudaMemcpy(mGPU->dzn5, zn, mGPU->nrows * sizeof(double), cudaMemcpyHostToDevice);
    }
#else
    cudaMemcpy((i * mGPU->nrows + mGPU->dzn), zn, mGPU->nrows * sizeof(double), cudaMemcpyHostToDevice);
#endif
  ...
  double cv_hprime3=cv_mem->cv_hprime;
  ...
  sd->mdv.cv_hprime2 = cv_mem->cv_hprime;
  sd->mdv.cv_hprime3 = cv_hprime3;
...
//#endif
  ...
  bicg->timeBiConjGrad+= msBiConjGrad;
    ...
    //cv_mem->cv_hprime = sd->mdv.cv_hprime2;//needed?
    cv_hprime3 = sd->mdv.cv_hprime3;
    ...
    *tret = sd->mdv.tret;
...
#ifndef DEV_DZN0
      if(i==0){
        cudaMemcpy(zn, mGPU->dzn0, mGPU->nrows * sizeof(double), cudaMemcpyDeviceToHost);
      }else if(i==1){
        cudaMemcpy(zn, mGPU->dzn1, mGPU->nrows * sizeof(double), cudaMemcpyDeviceToHost);
      }else if(i==2){
        cudaMemcpy(zn, mGPU->dzn2, mGPU->nrows * sizeof(double), cudaMemcpyDeviceToHost);
      }else if(i==3){
        cudaMemcpy(zn, mGPU->dzn3, mGPU->nrows * sizeof(double), cudaMemcpyDeviceToHost);
      }else if(i==4){
        cudaMemcpy(zn, mGPU->dzn4, mGPU->nrows * sizeof(double), cudaMemcpyDeviceToHost);
      }else if(i==5){
        cudaMemcpy(zn, mGPU->dzn5, mGPU->nrows * sizeof(double), cudaMemcpyDeviceToHost);
      }
#endif
    ...
    if ( istate==CV_SUCCESS ) {
      ...
      //Check why flag is not a success and prints the corresponding error from the list of situations where kernel can exit when flag!=SUCCESS
      if (kflag != CV_SUCCESS) {
        printf("cudaCVode2 kflag %d\n",kflag);
        istate = cvHandleFailure_gpu2(cv_mem, kflag);
        //cv_mem->cv_tretlast = *tret = cv_mem->cv_tn;
      ...
      //Copy all the ifs here to print the error correctly
    ...
    if ( cv_mem->cv_tstopset ) {
      ...
      if ( (cv_mem->cv_tn + cv_mem->cv_hprime - cv_mem->cv_tstop)*cv_mem->cv_h > ZERO ) {
        printf("cv_mem->cv_tn + cv_mem->cv_hprime - cv_mem->cv_tstop CPU\n");
        cv_mem->cv_hprime = (cv_mem->cv_tstop - cv_mem->cv_tn)*(ONE-FOUR*cv_mem->cv_uround);
        cv_mem->cv_eta = cv_mem->cv_hprime/cv_mem->cv_h;
      }
                                 ...
                                 realtype saved_t, int *nefPtr, realtype *dsmPtr)
{
  ...
  *nflagPtr = PREV_ERR_FAIL;
  ...
  if (*nefPtr <= MXNEF1) {
    cv_mem->cv_eta = ONE /
            (SUNRpowerR(BIAS2*dsm,ONE/cv_mem->cv_L) + ADDON);
...
void cvSetEta_gpu2(CVodeMem cv_mem)
{
  ...
  if (cv_mem->cv_eta < THRESH) {
    ...
    /* Limit eta by etamax and hmax, then set hprime */
    cv_mem->cv_eta = SUNMIN
            (cv_mem->cv_eta, cv_mem->cv_etamax);
    cv_mem->cv_eta /= SUNMAX(ONE,SUNRabs
    (cv_mem->cv_h)*cv_mem->cv_hmax_inv*cv_mem->cv_eta);
