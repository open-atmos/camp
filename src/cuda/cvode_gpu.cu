...
__device__
void cudaDevicecvAdjustParams(ModelDataGPU *md) {
...
__device__
void cudaDevicecvPredict(ModelDataGPU *md) {
...
__device__
void cudaDevicecvSet(ModelDataGPU *md) {
...
int cudaDevicecvHandleNFlag(ModelDataGPU *md, ModelDataVariable *dmdv, int *nflagPtr, double saved_t,
  ...
  *nflagPtr = PREV_CONV_FAIL;//1000 cells same result
...
void cudaGlobalSolveODE(ModelDataGPU md_object) {
  ...
  if(i==0) mdvo->ncf = dmdv->ncf;//todo delete (this should require check for failing cell to update, but since is generated in handlefailure maybe is not needed)
  mdvo->nef = dmdv->nef;
...
void cvRestore_gpu3(CVodeMem cv_mem, realtype saved_t)
{
  int j, k;
                       ...
                       int *ncfPtr)
{
  ...
  for (j=1; j <= cv_mem->cv_q; j++) {
    //N_VScale(factor, cv_mem->cv_zn[j], cv_mem->cv_zn[j]);
    printf("cvHandleNFlag_gpu3 factor %le j %d\n",factor,j);
    factor *= cv_mem->cv_eta;
  }//exit(0);
...
*/
    ...
    //if (nflag != DO_ERROR_TEST) cv_mem->cv_tn = saved_t;
...
*/
    ...
    //printf("DEV_cudacvStep nflag %d saved_t %le, sd->mdv.saved_t %le cv_mem->cv_tn %le sd->mdv.cv_tn %le\n",
    //      nflag,saved_t, sd->mdv.saved_t, cv_mem->cv_tn, sd->mdv.cv_tn);
...
int cvNlsNewton_gpu2(SolverData *sd, CVodeMem cv_mem, int nflag)
{
  ...
  for(;;) {
    ...
    //gpu_zaxpby(1.0, mGPU->dzn, 1.0, bicg->dacor_init, mGPU->dcv_y, mGPU->nrows, mGPU->blocks, mGPU->threads);
    ...
    //gpu_yequalsx(mGPU->dacor, bicg->dacor_init, mGPU->nrows, mGPU->blocks, mGPU->threads);
