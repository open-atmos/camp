 ...
 */
#ifndef AERO_RXN_CONDENSED_PHASE_ARRHENIUS_DEV_H_
#define AERO_RXN_CONDENSED_PHASE_ARRHENIUS_DEV_H_
...
__device__ void rxn_gpu_condensed_phase_arrhenius_calc_deriv_contrib(
  ...
  for (int i_phase = 0, i_deriv = 0; i_phase < int_data[2]; i_phase++) {
    ...
    if (int_data[3 + intd01 * int_data[2] + i_phase] - 1 >=
        0) {
      unit_conv = state[int_data[3 + intd01 * int_data[2] +
                                 i_phase] -
    ...
    for (int i_react = 0; i_react < int_data[0]; i_react++) {
      rate *= state[int_data[3 + (i_phase * int_data[0] + i_react)] - 1] *
              float_data[5 + int_data[1] + i_react] * unit_conv;
    }

    // Reactant change
    for (int i_react = 0; i_react < int_data[0]; i_react++) {
      if (int_data[3 + (intd01 + 1) * int_data[2] + i_deriv] < 0) {
        i_deriv++;
        continue;
      }
      time_derivative_add_value_gpu(
          time_deriv, int_data[3 + (intd01 + 1) * int_data[2] + i_deriv++],
          -rate / (float_data[5 + int_data[1] + i_react] * unit_conv));
    }
    ...
    for (int i_prod = 0; i_prod < int_data[1]; i_prod++) {
      ...
      time_derivative_add_value_gpu(
          time_deriv, int_data[3 + (intd01 + 1) * int_data[2] + i_deriv++],
          rate * float_data[5 + i_prod] /
              (float_data[5 + int_data[1] + (int_data[0] + i_prod)] *
               unit_conv));
    }
  }
...
__device__ void rxn_gpu_condensed_phase_arrhenius_calc_jac_contrib(
  ...
  int intd01 = int_data[0] + int_data[1];
  int int3012 = 3 + (2 * intd01 + 1) * int_data[2];
  ...
  for (int i_phase = 0, i_jac = 0; i_phase < int_data[2]; i_phase++) {
    ...
    for (int i_react_ind = 0; i_react_ind < int_data[0]; i_react_ind++) {
      ...
      for (int i_react_dep = 0; i_react_dep < int_data[0]; i_react_dep++) {
        ...
        jacobian_add_value_gpu(
            ...
            rate / (float_data[5 + int_data[1] + i_react_dep] * unit_conv));
      }
      ...
      for (int i_prod_dep = 0; i_prod_dep < int_data[1]; i_prod_dep++) {
        ...
        jacobian_add_value_gpu(
            ...
            rate * float_data[5 + i_prod_dep] /
                (float_data[5 + int_data[1] + (int_data[0] + i_prod_dep)] *
                 unit_conv));
      }
    }
    ...
    // aqueous reactions
    if (int_data[3 + intd01 * int_data[2] + i_phase] - 1 < 0) {
      i_jac += intd01;
      continue;
    }
    ...
    for (int i_react_dep = 0; i_react_dep < int_data[0]; i_react_dep++) {
      ...
      jacobian_add_value_gpu(jac, (unsigned int)int_data[int3012 + i_jac++], JACOBIAN_LOSS,
                         -(int_data[0] - 1) * rate /
                             float_data[5 + int_data[1] + i_react_dep]);
    }
    ...
    for (int i_prod_dep = 0; i_prod_dep < int_data[1]; i_prod_dep++) {
      ...
      jacobian_add_value_gpu(
          jac, (unsigned int)int_data[int3012 + i_jac++], JACOBIAN_PRODUCTION,
          -(int_data[0] - 1) * rate * float_data[5 + i_prod_dep] /
              float_data[5 + int_data[1] + (int_data[0] + i_prod_dep)]);
    }
  }
...
#endif // AERO_RXN_CONDENSED_PHASE_ARRHENIUS_DEV_H_