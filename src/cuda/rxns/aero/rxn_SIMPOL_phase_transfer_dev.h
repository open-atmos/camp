...
/** \file
 * \brief SIMPOL Phase Transfer reaction solver functions
 */
#ifndef AERO_RXN_SIMPOL_DEV_H_
#define AERO_RXN_SIMPOL_DEV_H_
...
__device__ void rxn_gpu_SIMPOL_phase_transfer_calc_deriv_contrib(
  ...
  for (int i_phase = 0; i_phase < int_data[0]; i_phase++) {
    ...
    if (int_data[2 + 4 * (int_data[0]) + 0] >= 0) {
      time_derivative_add_value_gpu(time_deriv,
                                    int_data[2 + 4 * (int_data[0]) + 0],
    ...
    // (kg/m^3/s)
    if (int_data[2 + 4 * (int_data[0]) + 1 + i_phase] >= 0) {
      time_derivative_add_value_gpu(
          time_deriv, int_data[2 + 4 * (int_data[0]) + 1 + i_phase],
...
__device__ void rxn_gpu_SIMPOL_phase_transfer_calc_jac_contrib(
  ...
  for (int i_phase = 0; i_phase < int_data[0]; i_phase++) {
    ...
    double radius;
    aero_rep_modal_binned_mass_get_effective_radius__m(
        int_data[2 + 2 * (int_data[0]) + i_phase] - 1, &radius,
    ...
    double d_cond_d_radius =
        gpu_d_gas_aerosol_transition_rxn_rate_constant_d_radius(
            DIFF_COEFF_, MFP_M_, radius, ALPHA_) *
        state[GAS_SPEC_];
    double d_evap_d_radius = d_cond_d_radius / state[GAS_SPEC_] * EQUIL_CONST_ *
    ...
    for (int i_elem = 0;
      ...
      if (int_data[int_data[4 + 10 * (int_data[0]) + i_phase] +
                   (0) * int_data[int_data[4 + 10 * (int_data[0]) + i_phase] -
                                  1] +
                   i_elem] > 0) {
      ...
      if (int_data[int_data[4 + 10 * (int_data[0]) + i_phase] +
                   (1) * int_data[int_data[4 + 10 * (int_data[0]) + i_phase] -
                                  1] +
                   i_elem] > 0) {
        // species involved in effective radius calculations
        jacobian_add_value_gpu(
