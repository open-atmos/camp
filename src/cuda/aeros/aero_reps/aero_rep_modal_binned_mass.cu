...
#include <stdio.h>
#include <stdlib.h>
#include "../aero_phase_gpu_solver.h"
...
__host__ __device__
#endif
int aero_rep_gpu_modal_binned_mass_get_used_jac_elem(ModelData *model_data,
                                                 int aero_phase_idx,
        ...
        if (aero_phase_idx == 0) {
          for (int j_phase = 0; j_phase < NUM_PHASE_(i_section); j_phase++) {
            PHASE_NUM_JAC_ELEM_(i_section, j_phase, i_bin) =
                    aero_phase_gpu_get_used_jac_elem(
...
__host__ __device__
#endif
void aero_rep_gpu_modal_binned_mass_update_env_state(ModelData *model_data,
                                                 int *aero_rep_int_data,
                                                 double *aero_rep_float_data,
...
__host__ __device__
#endif
void aero_rep_gpu_modal_binned_mass_update_state(ModelData *model_data,
                                             int *aero_rep_int_data,
                                             double *aero_rep_float_data,
                                             double *aero_rep_env_data) {
  int *int_data = aero_rep_int_data;
    ...
    switch (SECTION_TYPE_(i_section)) {
      ...
      case (MODAL):
        ...
        for (int i_phase = 0; i_phase < NUM_PHASE_(i_section); i_phase++) {
          ...
          aero_phase_gpu_get_mass__kg_m3(
          ...
          aero_phase_gpu_get_volume__m3_m3(
                  model_data, PHASE_MODEL_DATA_ID_(i_section, i_phase, 0), state,
                  &phase_volume, NULL);
          volume += phase_volume;
        }
        ...
        // volume (see aero_rep_gpu_modal_binned_mass_get_number_conc for details)
 ...
 * \param model_data Pointer to the model data, including the state array
 * \param aero_phase_idx Index of the aerosol phase within the representation
 * \param aero_phase_gpu_mass Total mass in the aerosol phase, \f$m\f$
 *                        (\f$\mbox{\si{\kilogram\per\cubic\metre}}\f$)
...
void aero_rep_gpu_modal_binned_mass_get_aero_phase_gpu_mass__kg_m3(
  ...
  for (int i_section = 0; i_section < NUM_SECTION_ && aero_phase_idx >= 0;
    ...
    for (int i_bin = 0; i_bin < NUM_BINS_(i_section) && aero_phase_idx >= 0;
      ...
      for (int i_phase = 0; i_phase < NUM_PHASE_(i_section); ++i_phase) {
        if (aero_phase_idx == 0) {
          *aero_phase_gpu_mass = PHASE_MASS_(i_section, i_phase, i_bin);
          if (partial_deriv) {
            // Get a pointer to the phase on the state array
            double *state = (double *)(model_data->grid_cell_state);
            state += PHASE_STATE_ID_(i_section, i_phase, i_bin);
 ...
 * \param model_data Pointer to the model data, including the state array
 * \param aero_phase_idx Index of the aerosol phase within the representation
 * \param aero_phase_gpu_avg_MW Average molecular weight in the aerosol phase
...
void aero_rep_gpu_modal_binned_mass_get_aero_phase_gpu_avg_MW__kg_mol(
  ...
  for (int i_section = 0; i_section < NUM_SECTION_ && aero_phase_idx >= 0;
    ...
    for (int i_bin = 0; i_bin < NUM_BINS_(i_section) && aero_phase_idx >= 0;
      ...
      for (int i_phase = 0; i_phase < NUM_PHASE_(i_section); ++i_phase) {
        if (aero_phase_idx == 0) {
          *aero_phase_gpu_avg_MW = PHASE_AVG_MW_(i_section, i_phase, i_bin);
          if (partial_deriv) {
            // Get a pointer to the phase on the state array
            double *state = (double *)(model_data->grid_cell_state);
            state += PHASE_STATE_ID_(i_section, i_phase, i_bin);
 ...
 *       pmc_aero_rep_gpu_modal_binned_mass::aero_rep_gpu_modal_binned_mass_t::set_id
