...
int aero_rep_gpu_single_particle_get_used_jac_elem(ModelDataGPU *model_data,
  ...
  for (int i_phase = 0; i_phase < NUM_PHASE_; ++i_phase) {
    PHASE_NUM_JAC_ELEM_(i_phase) = aero_phase_gpu_get_used_jac_elem(
...
void aero_rep_gpu_single_particle_get_effective_radius__m(
  ...
  *radius = 0.0;
  ...
  for (int i_phase = 0; i_phase < NUM_PHASE_; ++i_phase) {
    ...
    double volume;
    aero_phase_gpu_get_volume__m3_m3(model_data, PHASE_MODEL_DATA_ID_(i_phase),
...
void aero_rep_gpu_single_particle_get_aero_phase_gpu_mass__kg_m3(
  ...
  for (int i_phase = 0; i_phase < NUM_PHASE_; ++i_phase) {
    if (i_phase == aero_phase_idx) {
      ...
      state += i_part * PARTICLE_STATE_SIZE_ + PHASE_STATE_ID_(i_phase);
      double mw;
      aero_phase_gpu_get_mass__kg_m3(model_data, aero_phase_idx, state,
                                 aero_phase_gpu_mass, &mw, partial_deriv, NULL);
...
void aero_rep_gpu_single_particle_get_aero_phase_gpu_avg_MW__kg_mol(
  ...
  for (int i_phase = 0; i_phase < NUM_PHASE_; ++i_phase) {
    if (i_phase == aero_phase_idx) {
      ...
      state += i_part * PARTICLE_STATE_SIZE_ + PHASE_STATE_ID_(i_phase);
      double mass;
      aero_phase_gpu_get_mass__kg_m3(model_data, aero_phase_idx, state, &mass,
                                 aero_phase_gpu_avg_MW, NULL, partial_deriv);
      if (partial_deriv) partial_deriv += PHASE_NUM_JAC_ELEM_(i_phase);
    } else if (partial_deriv) {
      for (int i_spec = 0; i_spec < PHASE_NUM_JAC_ELEM_(i_phase); ++i_spec)
        *(partial_deriv++) = ZERO;
