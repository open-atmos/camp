...
__host__ __device__
#endif
int aero_rep_gpu_get_used_jac_elem(ModelData *model_data, int aero_rep_idx,
  ...
  switch (aero_rep_type) {
    case AERO_REP_MODAL_BINNED_MASS:
      num_flagged_elem = aero_rep_gpu_modal_binned_mass_get_used_jac_elem(
          model_data, aero_phase_idx, aero_rep_int_data, aero_rep_float_data,
          jac_struct);
      break;
    case AERO_REP_SINGLE_PARTICLE:
      num_flagged_elem = aero_rep_gpu_single_particle_get_used_jac_elem(
          model_data, aero_phase_idx, aero_rep_int_data, aero_rep_float_data,
          jac_struct);
      break;
  }
...
__host__ __device__
#endif
void aero_rep_gpu_get_dependencies(ModelData *model_data, bool *state_flags) {
  ...
  // used advancing the aero_rep_gpu_data pointer each time
  for (int i_aero_rep = 0; i_aero_rep < n_aero_rep; i_aero_rep++) {
    ...
    switch (aero_rep_type) {
      case AERO_REP_MODAL_BINNED_MASS:
        aero_rep_gpu_modal_binned_mass_get_dependencies(
            aero_rep_int_data, aero_rep_float_data, state_flags);
        break;
      case AERO_REP_SINGLE_PARTICLE:
        aero_rep_gpu_single_particle_get_dependencies(
            aero_rep_int_data, aero_rep_float_data, state_flags);
        break;
    }
  }
...
__host__ __device__
#endif
void aero_rep_gpu_update_env_state(ModelData *model_data) {
  // Get the number of aerosol representations
  int n_aero_rep = model_data->n_aero_rep;

  // Loop through the aerosol representations to update the environmental
  // conditions advancing the aero_rep_gpu_data pointer each time
  for (int i_aero_rep = 0; i_aero_rep < n_aero_rep; i_aero_rep++) {
    // Get pointers to the aerosol data
    int *aero_rep_int_data = &(
        model_data
    ...
    switch (aero_rep_type) {
      case AERO_REP_MODAL_BINNED_MASS:
        aero_rep_gpu_modal_binned_mass_update_env_state(
            model_data, aero_rep_int_data, aero_rep_float_data,
            aero_rep_env_data);
        break;
      case AERO_REP_SINGLE_PARTICLE:
        aero_rep_gpu_single_particle_update_env_state(model_data, aero_rep_int_data,
                                                  aero_rep_float_data,
                                                  aero_rep_env_data);
        break;
    }
...
__host__ __device__
#endif
void aero_rep_gpu_update_state(ModelData *model_data) {
  // Get the number of aerosol representations
  int n_aero_rep = model_data->n_aero_rep;
...
__host__ __device__
#endif
void aero_rep_gpu_get_effective_radius__m(ModelData *model_data, int aero_rep_idx,
...
__host__ __device__
#endif
void aero_rep_gpu_get_number_conc__n_m3(ModelData *model_data, int aero_rep_idx,
 ...
 * \param aero_phase_idx Index of the aerosol phase within the aerosol
 *                       representation
 * \param aero_phase_gpu_mass Pointer to hold calculated aerosol-phase mass,
 *                        \f$m\f$
...
__host__ __device__
#endif
void aero_rep_gpu_get_aero_phase_gpu_mass__kg_m3(ModelData *model_data,
  ...
  switch (aero_rep_type) {
    case AERO_REP_MODAL_BINNED_MASS:
      aero_rep_gpu_modal_binned_mass_get_aero_phase_gpu_mass__kg_m3(
          model_data, aero_phase_idx, aero_phase_gpu_mass, partial_deriv,
          aero_rep_int_data, aero_rep_float_data, aero_rep_env_data);
      break;
    case AERO_REP_SINGLE_PARTICLE:
      aero_rep_gpu_single_particle_get_aero_phase_gpu_mass__kg_m3(
          model_data, aero_phase_idx, aero_phase_gpu_mass, partial_deriv,
          aero_rep_int_data, aero_rep_float_data, aero_rep_env_data);
      break;
  }
 ...
 * \param aero_phase_idx Index of the aerosol phase within the aerosol
 *                       representation
 * \param aero_phase_gpu_avg_MW Pointer to hold calculated average MW in the
...
__host__ __device__
#endif
void aero_rep_gpu_get_aero_phase_gpu_avg_MW__kg_mol(ModelData *model_data,
                                            int aero_rep_idx,
                                            int aero_phase_idx,
  ...
  switch (aero_rep_type) {
    case AERO_REP_MODAL_BINNED_MASS:
      aero_rep_gpu_modal_binned_mass_get_aero_phase_gpu_avg_MW__kg_mol(
          model_data, aero_phase_idx, aero_phase_gpu_avg_MW, partial_deriv,
          aero_rep_int_data, aero_rep_float_data, aero_rep_env_data);
      break;
    case AERO_REP_SINGLE_PARTICLE:
      aero_rep_gpu_single_particle_get_aero_phase_gpu_avg_MW__kg_mol(
          model_data, aero_phase_idx, aero_phase_gpu_avg_MW, partial_deriv,
          aero_rep_int_data, aero_rep_float_data, aero_rep_env_data);
      break;
  }
...
void aero_rep_gpu_update_data(int cell_id, int *aero_rep_id,
  ...
  for (; (*aero_rep_id) < n_aero_rep; (*aero_rep_id)++) {
    ...
    if (aero_rep_type == update_aero_rep_type) {
      switch (aero_rep_type) {
        case AERO_REP_MODAL_BINNED_MASS:
          found = aero_rep_gpu_modal_binned_mass_update_data(
              (void *)update_data, aero_rep_int_data, aero_rep_float_data,
              aero_rep_env_data);
          break;
        case AERO_REP_SINGLE_PARTICLE:
          found = aero_rep_gpu_single_particle_update_data(
              (void *)update_data, aero_rep_int_data, aero_rep_float_data,
              aero_rep_env_data);
          break;
      }
