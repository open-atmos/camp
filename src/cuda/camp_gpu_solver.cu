...
/*
    printf("md->n_per_cell_dep_var %d sd->jac.num_spec %d "
           "md->n_per_cell_solver_jac_elem %d "
           "md->n_mapped_values %d SM_NNZ_S(md->J_rxn) %d offset_nnz_J_solver %d "
           "mGPU->nnz_J_solver %d mGPU->nrows_J_solver %d"
           "  md->n_per_cell_state_var %d"
           " sd->jac.num_elem %d \n",
           ...
           SM_NNZ_S(sd->J),md->n_per_cell_state_var,
        sd->jac.num_elem
    );
    exit(0);
*/
