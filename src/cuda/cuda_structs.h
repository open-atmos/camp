...
typedef struct {
  ...
  int cv_jcur;    // flag indicating if Jacobian info. for lin. solver
                  // is current
...
typedef struct {
  ...
  double *
      state;  // Concentrations of species, including constant and non-constant
              // values. Used as input and output of the ODE solver. Derivatives
              // = Concentrations of non-constant species. The ODE solver works
