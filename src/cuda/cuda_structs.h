...
typedef struct {
  ...
  double dtPostBCG;
#endif
} ModelDataVariable;  // variables to pass between gpu and cpu (different data
                      // between cells)

typedef struct {
  ModelDataVariable mdvCPU;  // WARNING: Moving this to struct "sd" cause errors
                             // when running gpu version
} ModelDataCPU;
