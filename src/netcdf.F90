...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine camp_nc_check(status)
  ...
  end subroutine camp_nc_check

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Check the status of a NetCDF function call and prints the given
  !> error message on failure.
  subroutine camp_nc_check_msg(status, error_msg)
  ...
  end subroutine camp_nc_check_msg

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Open a NetCDF file for reading.
  subroutine camp_nc_open_read(filename, ncid)
    ...
    call camp_nc_check_msg(nf90_open(filename, NF90_NOWRITE, ncid), &
         "opening " // trim(filename) // " for reading")

  end subroutine camp_nc_open_read

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Open a NetCDF file for writing.
  subroutine camp_nc_open_write(filename, ncid)
    ...
    call camp_nc_check_msg(nf90_create(filename, NF90_CLOBBER, ncid), &
         "opening " // trim(filename) // " for writing")
    call camp_nc_check(nf90_enddef(ncid))

  end subroutine camp_nc_open_write

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Close a NetCDF file.
  subroutine camp_nc_close(ncid)

    !> NetCDF file ID, in data mode.
    integer, intent(in) :: ncid

    call camp_nc_check_msg(nf90_close(ncid), "closing NetCDF file")

  end subroutine camp_nc_close

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Write basic information to a NetCDF file.
  subroutine camp_nc_write_info(ncid, uuid, source, write_rank, write_n_proc)
    ...
    call camp_nc_check(nf90_put_att(ncid, NF90_GLOBAL, "title", &
         trim(source) // " output file"))
    call camp_nc_check(nf90_put_att(ncid, NF90_GLOBAL, "source", source))
    call camp_nc_check(nf90_put_att(ncid, NF90_GLOBAL, "UUID", uuid))
    call iso8601_date_and_time(history)
    history((len_trim(history)+1):) = (" created by " // trim(source))
    call camp_nc_check(nf90_put_att(ncid, NF90_GLOBAL, "history", history))
    call camp_nc_check(nf90_put_att(ncid, NF90_GLOBAL, "Conventions", "CF-1.4"))
    call camp_nc_check(nf90_enddef(ncid))
    ...
    call camp_nc_write_integer(ncid, use_n_proc, "total_processes", &
         description="total number of processes")
#endif

  end subroutine camp_nc_write_info

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Read a single real from a NetCDF file.
  subroutine camp_nc_read_real(ncid, var, name, must_be_present)
    ...
    call camp_nc_check_msg(status, "inquiring variable " // trim(name))
    call camp_nc_check_msg(nf90_get_var(ncid, varid, var), &
         "getting variable " // trim(name))

  end subroutine camp_nc_read_real

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Read a single integer from a NetCDF file.
  subroutine camp_nc_read_integer(ncid, var, name, must_be_present)
  ...
  end subroutine camp_nc_read_integer

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Read a simple real array from a NetCDF file.
  subroutine camp_nc_read_real_1d(ncid, var, name, must_be_present)
    ...
    call camp_nc_check_msg(status, "inquiring variable " // trim(name))
    call camp_nc_check_msg(nf90_inquire_variable(ncid, varid, dimids=dimids), &
         "determining size of variable " // trim(name))
    call camp_nc_check_msg(nf90_inquire_dimension(ncid, dimids(1), len=size1), &
  ...
  end subroutine camp_nc_read_real_1d

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Read a simple integer array from a NetCDF file.
  subroutine camp_nc_read_integer_1d(ncid, var, name, must_be_present)
  ...
  end subroutine camp_nc_read_integer_1d

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Read a simple real 2D array from a NetCDF file.
  subroutine camp_nc_read_real_2d(ncid, var, name, must_be_present)
    ...
    call camp_nc_check_msg(nf90_inquire_dimension(ncid, dimids(1), len=size1), &
         "determining size of dimension number " &
         // trim(integer_to_string(dimids(1))))
    call camp_nc_check_msg(nf90_inquire_dimension(ncid, dimids(2), len=size2), &
         "determining size of dimension number " &
  ...
  end subroutine camp_nc_read_real_2d

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Read a simple integer 2D array from a NetCDF file.
  subroutine camp_nc_read_integer_2d(ncid, var, name, must_be_present)
  ...
  end subroutine camp_nc_read_integer_2d

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Write attributes for a variable to a NetCDF file.
  subroutine camp_nc_write_atts(ncid, varid, unit, long_name, standard_name, &
       description)
    ...
    if (present(unit)) then
       call camp_nc_check(nf90_put_att(ncid, varid, "unit", unit))
    end if
    if (present(long_name)) then
       call camp_nc_check(nf90_put_att(ncid, varid, "long_name", long_name))
    end if
    if (present(standard_name)) then
       call camp_nc_check(nf90_put_att(ncid, varid, "standard_name", &
            standard_name))
    end if
    if (present(description)) then
       call camp_nc_check(nf90_put_att(ncid, varid, "description", &
            description))
    end if

  end subroutine camp_nc_write_atts

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Write a single real to a NetCDF file.
  subroutine camp_nc_write_real(ncid, var, name, unit, long_name, &
    ...
    call camp_nc_check(nf90_redef(ncid))
    call camp_nc_check(nf90_def_var(ncid, name, NF90_DOUBLE, dimids, varid))
    call camp_nc_write_atts(ncid, varid, unit, long_name, standard_name, &
         description)
    call camp_nc_check(nf90_enddef(ncid))

    call camp_nc_check(nf90_put_var(ncid, varid, var))

  end subroutine camp_nc_write_real

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Write a single integer to a NetCDF file.
  subroutine camp_nc_write_integer(ncid, var, name, unit, long_name, &
       standard_name, description)
    ...
    call camp_nc_check(nf90_redef(ncid))
    call camp_nc_check(nf90_def_var(ncid, name, NF90_INT, dimids, varid))
    call camp_nc_write_atts(ncid, varid, unit, long_name, standard_name, &
         description)
    call camp_nc_check(nf90_enddef(ncid))
  ...
  end subroutine camp_nc_write_integer

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Create a dimension if necessary, or check its size if it already
  !> exists. In any case return the \c dimid.
  subroutine camp_nc_ensure_dim(ncid, dim_name, dimid, dim_size, array_dim)
    ...
    if (status == NF90_NOERR) then
       call camp_nc_check(nf90_Inquire_Dimension(ncid, dimid, check_name, &
            check_dim_size))
       ...
       call camp_nc_check(nf90_redef(ncid))
       call camp_nc_check(nf90_def_dim(ncid, dim_name, dim_size, dimid))
       call camp_nc_check(nf90_enddef(ncid))
    end if

  end subroutine camp_nc_ensure_dim

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Write a simple real array to a NetCDF file.
  subroutine camp_nc_write_real_1d(ncid, var, name, dimids, dim_name, unit, &
    ...
    elseif (present(dim_name)) then
       call camp_nc_ensure_dim(ncid, dim_name, use_dimids(1), size(var), 1)
    else
       call die_msg(891890123, "either dimids or dim_name must be present")
    end if
    call camp_nc_check(nf90_redef(ncid))
    call camp_nc_check(nf90_def_var(ncid, name, NF90_DOUBLE, use_dimids, varid))
    call camp_nc_write_atts(ncid, varid, unit, long_name, standard_name, &
         description)
    call camp_nc_check(nf90_enddef(ncid))
  ...
  end subroutine camp_nc_write_real_1d

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Write a simple integer array to a NetCDF file.
  subroutine camp_nc_write_integer_1d(ncid, var, name, dimids, dim_name, unit, &
       long_name, standard_name, description)
    ...
    call camp_nc_check(nf90_redef(ncid))
    call camp_nc_check(nf90_def_var(ncid, name, NF90_INT, use_dimids, varid))
    call camp_nc_write_atts(ncid, varid, unit, long_name, standard_name, &
         description)
    call camp_nc_check(nf90_enddef(ncid))
  ...
  end subroutine camp_nc_write_integer_1d

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Write a simple real 2D array to a NetCDF file.
  subroutine camp_nc_write_real_2d(ncid, var, name, dimids, dim_name_1, &
    ...
    elseif (present(dim_name_1) .and. present(dim_name_2)) then
       call camp_nc_ensure_dim(ncid, dim_name_1, use_dimids(1), size(var, 1), 1)
       call camp_nc_ensure_dim(ncid, dim_name_2, use_dimids(2), size(var, 2), 2)
    else
  ...
  end subroutine camp_nc_write_real_2d

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !> Write a simple integer 2D array to a NetCDF file.
  subroutine camp_nc_write_integer_2d(ncid, var, name, dimids, dim_name_1, &
       dim_name_2, unit, long_name, standard_name, description)
  ...
  end subroutine camp_nc_write_integer_2d

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

end module camp_netcdf
