      ...
      */
      ...
      switch (rxn_type) {
        ...
        case RXN_HL_PHASE_TRANSFER:
          ...
          break;
        case RXN_PHOTOLYSIS:
          rxn_photolysis_calc_deriv_contrib(model_data, deriv_data,
                                            rxn_int_data, rxn_float_data,
                                            rxn_env_data, time_step);
          break;
        case RXN_SIMPOL_PHASE_TRANSFER:
      ...
      // todo test time_deriv with all specs fom all cells, test with only
      // r_p-r_l, and compare with original
...
/** \brief DEPRECATED Calculate the time derivative \f$f(t,y)\f$ for only some specific
 * types
 *
 * \param model_data Pointer to the model data
 * \param time_deriv TimeDerivative to use to build derivative array
...
void rxn_export_input(void *solver_data, FILE *f) {
  ...
  for (int i_rxn = 0; i_rxn < n_rxn; i_rxn++) {
    ...
    for (int i_cell = 0; i_cell < model_data->n_cells; i_cell++) {
      ...
      switch (rxn_type) {
        case RXN_PHOTOLYSIS:
          rxn_photolysis_export_input(model_data, rxn_int_data, rxn_float_data,
                                      rxn_env_data, f);
          break;
      }
    }
...
void rxn_get_base_rate(void *solver_data, double *rate_constants) {
  ...
  //printf("base_rate -le%", rate_constants[1]);
  ...
  for (int i_rxn = 0; i_rxn < n_rxn; i_rxn++) {
    ...
    for (int i_cell = 0; i_cell < model_data->n_cells; i_cell++) {
      ...
      switch (rxn_type) {
        case RXN_PHOTOLYSIS:
          rxn_photolysis_get_base_rate(model_data, rxn_int_data, rxn_float_data,
                                      rxn_env_data, &(rate_constants[i]));
          i++;
          //rate_constants[i_rxn]= rate_constant;
          break;
      }
    }
  }
